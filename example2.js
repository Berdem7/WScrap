      ...
      axios
        ...
        .then(({ data }) => {
          ...
          if (
            ...
            //   words.push(codes[i]);
            console.log(codes[i] + " bol ug bish yumaa");
          } else {
            //   nonwords.push(codes[i]);
            console.log(codes[i] + " bol ug yumaa");
            let file = { index: i, word: codes[i] };
            fs.readFile("words.json", function (err, data) {
              let json = JSON.parse(data);
              console.log(json);
              json.push(file);
              fs.writeFile("words.json", JSON.stringify(json), function (err) {
                if (err) {
                  throw err;
                } else {
                  console.log("success");
